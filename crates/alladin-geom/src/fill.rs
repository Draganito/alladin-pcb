...
//! crate's polygon boolean/buffer engine -- what copper pour zones
//! need and nothing else in this workspace needs: union/intersection/
//! difference of arbitrary polygons, and buffering (offsetting) a
//! polygon outward by a clearance distance -- the two primitives a real
//! zone-fill algorithm is built from (clip to the board outline,
//! inflate every obstacle by its clearance, union them, subtract from
//! the zone outline).
//!
//! # Why `i_overlay`
//! Pure Rust, actively maintained, and -- uniquely among the
//! alternatives surveyed -- offers both boolean ops *and*
...
impl FilledRegion {
    ...
    /// confirmed on a real board), and what lets a hole-bearing fill
    /// result still fit
