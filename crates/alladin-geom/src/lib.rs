...
/// Today's only consumer is the board outline (`Edge.Cuts`), validated
/// against real boards (e.g. a 9-edge non-convex outline chained from
/// individually-unordered `gr_line` forms).
...
/// outline data doesn't guarantee either winding direction, so anything
...
/// out itself rather than assume one.
    ...
    /// waypoints for clearance/geometry queries that need to hug a
    /// concave stretch of this polygon's own boundary, where no nearby
    /// obstacle happens to seed one.
    ///
...
/// a proper miter-based offset would be the fix if a pathological
/// board outline ever needs it.
...
/// Approximate clearance-inflated boundary of `self`, for seeding
/// collision/clearance candidate points around a filled zone treated
/// as an obstacle -- the polygon-shaped counterpart to circle/capsule
...
///    cuts inside the true curve" issue that circular-arc safety
///    factors address elsewhere, just for a polygon corner instead of
///    a circular arc. Skipped
...
/// algorithm doesn't guarantee one): a point
...
/// `outline.is_empty() || ...` -- deliberately kept as the caller's
/// choice, not baked in here, since it's a routing-domain default, not
/// a geometric one).
...
/// used elsewhere for "is this round shape actually clear", just
/// re-purposed here for on-board containment instead of obstacle
/// clearance.
...
/// convex-boundary tangent-point queries (clearance geometry), not just
/// the segment-intersection test below.
...
/// (a real 109-LED panel's 5V pour: 42,415). Dense clearance checks
/// call the equivalent of a collision check thousands of times *per
/// candidate*, so on a route whose corridor overlaps such a zone,
/// that `O(n)` scan was being paid thousands of times over -- easily
/// billions of edge-distance evaluations for a single net, which is
/// what silently hung dense clearance checks on real boards with large
/// zone fills. Building one `rstar` R-tree of a zone's edges
/// once and reusing it for every one of those checks turns each check
/// into an `O(log n)`-ish spatially
...
mod tests {
    ...
    fn segments_intersect_ignores_collinear_points_beyond_the_segment_extent() {
        ...
        // segment. Found when a vertical obstacle segment from
        // (2.5mm, -1mm) to (2.5mm, 1mm)
    ...
    /// / `SHAPE_POLY_SET::Contains()` -- chained together from
    /// individually-unordered `gr_line` forms.
    ...
    fn evenodd_excludes_a_hole_nested_inside_a_boundary() {
        ...
        // internal-cutout combination produces.
