...
fn parse_pad_line(line: &str, origin_x: f64, origin_y: f64) -> Option<PadTemplate> {
    ...
    // (see the KiCad importer doc's layer table). A through-hole pad
    ...
    // circle), not the bounding box's half-diagonal, and not even
    // `alladin-kicad-io`'s own `max(w, h) / 2` for real, imported KiCad
    // rect pads. Both of those were tried first and were real bugs on a
    ...
    // trips into a real KiCad `thru_hole` pad with its actual drill
    // size on export (`crate::kicad_export`) instead of silently
    // becoming an unmanufacturable SMD pad.
...
mod tests {
    ...
    fn a_real_esp32_wroom_32_s_edge_pads_do_not_overlap_their_own_neighbours() {
        ...
        // (in practice: a pathologically slow walkaround search). Root
