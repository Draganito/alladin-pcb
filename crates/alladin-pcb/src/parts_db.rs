//! The user's own SQL parts library -- see the development log's
//! "Teil 29" entry: "die Software führt ihre eigene SQL
//! Parts-Datenbank...". Backed by SQLite (`rusqlite`, bundled -- no
//! system library dependency) at a per-user data directory, so it
//! persists across every board, independent of any single `.json` save
//! file.
//!
//! This first slice is deliberately **not** the LCSC/EasyEDA C-number
//! downloader from the original vision -- that's a real HTTP client
//! against a third-party, partially-reverse-engineered API plus a
//! converter from EasyEDA's own footprint format, substantial enough to
//! be its own step (see the development log's "Teil 29" MVP-order
//! entry: parts-database *access* comes before that specific API
//! integration). What this does provide: a real, persistent SQL-backed
//! library a user can register their own simple through-hole parts into
//! by hand today, that "download by C-number" can slot into tomorrow
//! without changing anything about how parts are stored or placed.
//!
//! A [`PartRecord`] is, deliberately, little more than a persisted
//! [`FootprintTemplate`] plus the two bits of metadata a template itself
//! has no room for (`lcsc_code`, `description`) -- everything that
//! already knows how to place/drag/route/save a `FootprintTemplate`
//! keeps working unchanged for a database-backed one.
...
pub struct PartRecord {
    ...
    /// `"Resistors"` (an LCSC download's own EasyEDA tag, see
    /// `crate::lcsc::FetchedPart::category`) or
    /// `"Imported files/darkroom_led_panel"` (a KiCad import, see
    /// `crate::kicad_import::persist_templates` -- the `/` is what
    /// makes it a *sub*-category of "Imported files" rather than its
    /// own top-level bucket). `None` for a part inserted before this
    /// column existed, or one that was never given a category at all
    /// (hand-registered parts before this feature, or via a still-empty
    /// `AddPartForm::category`) -- the GUI groups those under a plain
    /// "Uncategorized" bucket rather than hiding them.
...
impl PartsDb {
    ...
    /// e.g. "has holes but no pads", so a caller like `crate::cli`'s
    /// `register-part` can opt a pads-and-holes part out of the BOM
    /// too if it genuinely isn't purchasable). Refuses a second part
    /// under the same non-`None` `lcsc_code` -- see
    /// [`PartsDbError::DuplicateLcscCode`].
    ///
    /// Production callers all use [`Self::insert_part_categorized`]
    /// directly by now; this shorthand survives for the many tests
    /// (here and in `crate::cli`) that exercise the library through it.
    ...
    /// a new required parameter on [`Self::insert_part`] itself so
    /// every existing caller (CLI/GUI/MCP `register-part`, every
    /// hand-added part, every existing test) keeps compiling
    /// unchanged, correctly falling back to
    /// [`FootprintTemplate::courtyard`]'s own pad/hole bounding-box
    /// fallback via a plain `None` here.
    #[cfg_attr(not(test), allow(dead_code))]
    ...
    /// references it by `template_origin` -- or forcing a
    /// delete-then-reinsert round trip through the caller (see
    /// `crate::cli`'s `update-part` subcommand, the one caller
    /// today). Refuses with a plain SQLite "no rows" error if
    /// `lcsc_code` isn't already in the library -- this is
    /// deliberately never how a *new* part gets added.
    #[allow(clippy::too_many_arguments)]
