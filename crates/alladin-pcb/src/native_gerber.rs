...
/// Soldermask expansion beyond copper pad size. KiCad 9's plot of our
/// own `.kicad_pcb` uses **0** (mask opening == pad size) when no
...
/// KiCad's Gerber/position plot negates board-Y relative to the
/// `.kicad_pcb` coordinate system (verified: every via/pad in a
/// `kicad-cli pcb export gerbers` of our own export has `Y' = -Y`).
/// Matching that convention keeps native Gerbers + CPL drop-in
/// compatible with boards previously ordered via the kicad-cli path.
...
fn build_gerber_files(doc: &BoardDoc, templates: &[FootprintTemplate], stem: &str) -> Vec<NamedFile> {
    ...
    for item in doc.node.iter() {
        match item {
            ...
            // (same source `kicad_export` uses). Skipping `Item::Hole`
    ...
    for placed in &doc.footprints {
        ...
        for pad in &template.pads {
            ...
            // negated board angle (same as `kicad_export`'s written `at`).
...
fn build_jlcpcb_cpl(doc: &BoardDoc, templates: &[FootprintTemplate]) -> String {
    ...
    for fp in rows {
        ...
        let layer = templates
            ...
            .unwrap_or("Top");
        // Match `kicad-cli pcb export pos`: Y negated vs board coords,
        // rotation negated vs Alladin's internal angle (equals the angle
        // written into `.kicad_pcb`). Bottom parts get an extra mirror
        // convention already baked into KiCad's pos `Rot` — we keep the
        // same single negation for both sides, which matched KiCad's
        // top-side output on the LED-panel board.
