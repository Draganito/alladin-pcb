...
//! `alladin_core::Item::Zone` fill islands -- Feature B2 of
//! the development log's "Vias und Kupferflächen im Editor" plan.
//!
...
//! underneath it, which contradicts this project's "no compromises on
//! real geometry" line (see the "Echte Pad-Geometrie" work this plan's
//! own context section points back to). Instead, [`fill_zone`] runs the
//! same board-clip / obstacle-buffer / union / difference pipeline a
//! real copper pour needs, using [`alladin_geom::fill`]'s `i_overlay`
//! adapter for the actual polygon boolean/buffer math.
//!
//! Deliberately **not** modelled in this first slice (same scope cut
//! `alladin_kicad_io::import_zone` already documents for an *imported*
//! zone): thermal-relief spokes (same-net pads/vias are simply left
//! solidly connected, no spoke gap), and priority between multiple
//! zones of different nets overlapping each other (an existing
//! [`Item::Zone`] is not itself treated as an obstacle here -- only
//! `Pad`/`Via`/`Track`).
...
/// Deliberately higher than `alladin-router::astar`'s `CIRCLE_SEGMENTS`
/// (24, chosen for pathfinding-graph cost): zone filling runs once per
/// explicit user action, never in a search hot loop, so it can afford
/// the extra smoothness without any real performance cost.
...
/// disjoint filled island, exactly mirroring how
/// `alladin_kicad_io::import_zone` already turns a real KiCad
/// `filled_polygon` list into one `Item::Zone` per entry. Returns an
