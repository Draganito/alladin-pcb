//! Save/load for a [`BoardDoc`], as Aladin PCB's own JSON format --
//! **not** `.kicad_pcb` yet. A real KiCad writer needs to emit valid
//! footprint/pad S-expressions (library references or embedded
//! definitions, net tables, board setup) which is a project of its own;
//! see the development log's "Teil 29" entry, where Gerber/BOM/KiCad
//! export was explicitly deferred to a later phase. Until then, a board
//! that can't be saved and reopened at all isn't a usable editor, so this
//! lightweight format closes that gap first.
//!
...
//! other shape of `Item::Zone`: `crate::kicad_import`'s "already-filled,
//! static, no `ZoneRecord` behind it, never reshaped" copper pours read
//! straight out of a real `.kicad_pcb`'s `filled_polygon`s (see that
//! module's own doc comment). Before this version, [`to_json`] only ever
//! looked at `doc.zones` (the `ZoneRecord` list) to decide what counted
//! as a zone at all -- an imported board's *actual ground/power plane
//! copper*, sitting in `doc.node` with no `ZoneRecord` pointing at it,
//! was silently dropped by every single save, surviving only until the
//! process holding it in memory closed. [`to_json`] now also walks
//! `doc.node` directly for any `Item::Zone` not already accounted for by
//! a tracked `ZoneRecord::item_ids`, and [`from_json`] restores those
//! straight back into `node` verbatim (no `fill_zone` re-run -- there
//! never was a source outline to refill from).
...
/// One frozen copper-pour island with no [`crate::board_doc::ZoneRecord`]
/// behind it -- see this module's doc comment for `FORMAT_VERSION` 4 and
/// `crate::kicad_import`'s own doc comment for where these come from.
/// Unlike [`SavedZone`], `net` is genuinely optional (a real KiCad zone
/// can target no net at all) and there is no outline to refill from on
/// load -- the polygon saved here *is* the exact, final copper shape.
#[derive(Serialize, Deserialize)]
...
pub fn to_json(doc: &BoardDoc) -> String {
...
/// directly from their saved geometry. `templates` must contain every
/// template any saved footprint might reference by name -- in practice,
/// the caller's built-in templates plus whatever is currently in
/// `crate::parts_db::PartsDb` (a board can reference a database-backed
/// part, so a fixed built-in-only list isn't enough here).
pub fn from_json(json: &str, templates: &[FootprintTemplate]) -> Result<BoardDoc, LoadError> {
    ...
    for (index, sf) in saved.footprints.into_iter().enumerate() {
        let template = templates
            .iter()
            .find(|t| t.name == sf.template_name)
            .ok_or_else(|| LoadError::UnknownTemplate(sf.template_name.clone()))?;
    ...
    // `crate::kicad_import` established for these in the first place
    // (see `SavedStaticZoneIsland`'s doc comment).
...
mod tests {
    ...
    fn round_trips_an_empty_board() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a freshly-created board must round-trip");
    ...
    fn round_trips_a_placed_silk_text() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with a placed silk text must round-trip");
        ...
        assert!(!empty_board_json.contains("silk_texts") || from_json(&empty_board_json, &builtin_templates()).unwrap().silk_texts.is_empty());
    }
    ...
    fn round_trips_a_placed_silk_dot_and_a_pin1_marker() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("dots and markers must round-trip");
        ...
        let empty_board_json = to_json(&NewBoardParams::default().create());
        assert!(from_json(&empty_board_json, &builtin_templates()).unwrap().silk_dots.is_empty());
    }
    ...
    fn round_trips_footprints_with_their_nets_and_a_routed_track() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with parts/nets/tracks must round-trip");
    ...
    fn round_trips_a_manually_placed_via() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with a via must round-trip");
    ...
    fn round_trips_a_drawn_zone_restoring_its_saved_fill_verbatim() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with a zone must round-trip");
    ...
    fn a_save_from_before_zone_islands_existed_still_refills_on_load() {
        ...
        let loaded = from_json(&legacy, &builtin_templates()).expect("a legacy outline-only zone save must still load");
        assert_eq!(loaded.zones.len(), 1);
    ...
    fn a_zone_that_was_stale_at_save_time_is_still_stale_after_loading() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with a stale zone must round-trip");
    ...
    #[test]
    fn round_trips_a_static_zone_island_with_no_zonerecord_behind_it_like_a_kicad_import_produces() {
        // Simulates exactly what `crate::kicad_import::import_kicad_pcb`
        // hands back for a real KiCad zone: an `Item::Zone` sitting
        // directly in `node`, with `doc.zones` (the `ZoneRecord` list)
        // left completely empty -- before `FORMAT_VERSION` 4, `to_json`
        // only ever looked at `doc.zones`, so this exact shape was
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a board with a static zone island must round-trip");
    ...
    fn a_static_zone_island_coexists_with_a_tracked_zonerecord_without_either_duplicating_or_swallowing_the_other() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).unwrap();
    ...
    fn round_trips_a_2oz_boards_copper_weight() {
        ...
        let loaded = from_json(&to_json(&doc), &builtin_templates()).expect("a 2oz board must round-trip");
    ...
    fn a_save_from_before_copper_weight_existed_loads_as_1oz() {
        ...
        let loaded = from_json(&without_copper_weight, &builtin_templates()).expect("a save missing copper_weight must still load");
    ...
    fn resolves_a_footprint_against_a_non_builtin_template_list() {
        ...
        let loaded = from_json(&to_json(&doc), std::slice::from_ref(&custom)).expect("passing the custom template in must resolve it");
