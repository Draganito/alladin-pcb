...
//! `alladin-kicad-io`'s `import_zone` doc comment) -- re-tessellating that
//! into a fill every single frame is real, avoidable per-frame cost. A
    ...
    if layers.tracks {
        for item in items {
            if let Item::Track { shape, net, layer, .. } = item {
                ...
                // `alladin_geom::dist_point_to_segment`/
                // `alladin_router::capsule_walkaround`). Filling in a
