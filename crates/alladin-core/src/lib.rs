...
//!
//! This module is the Rust equivalent of what we reverse-engineered from
//! KiCad's `pcbnew/router/pns_node.{h,cpp}` and `pns_item.h` this evening
//! (see the development log) -- **the algorithm's shape, not its code**.
//! Concretely:
//!
...
//! (structurally tied to `KIGFX::VIEW_ITEM` + `wxPropertyGrid` -- see
//! the development log "Update 2026-07-29 (Nacht)"). Alladin's `Item` is a plain
//! data enum with zero rendering/property-panel baggage.
...
pub enum NetClass {
    ...
    B,
    /// Ordinary GPIO/signal: flexible, may be shoved by later passes.
    C,
}
...
/// compromise approximation of it. Before this type existed, every
/// [`Item::Pad`] was unconditionally a [`Circle`] with radius
/// `min(width, height) / 2` for non-round footprints (SOIC/QFN/WROOM
/// castellation pads, ...) -- an *inscribed*-circle shortcut smaller
/// than the pad's true half-extent along its long axis, which is
/// exactly why a route could pass through real copper near such a
/// pad's corner while the (too-small) collision circle read as clear.
/// See the development log's "Echte Pad-Geometrie" slice entry for the
/// full story and why [`Item::Via`] deliberately keeps a plain
/// [`Circle`] instead (vias are always round).
...
/// [`Node::remove`] and `alladin-router`'s SHOVE, the only two spots
...
pub enum Item {
    ...
    Via {
        ...
        /// ratio, not the 0.5 this crate's importer/exporter used to
        /// assume before this field existed; see the development log's
        /// corresponding update). Purely informational for
    ...
    /// re-fills nor live-updates it as new tracks are routed (see
    /// `alladin-kicad-io`'s `import_zone` doc comment for the full list
    /// of deliberately unmodeled zone behaviour: thermal-relief spokes,
    /// zone-priority between overlaps, etc.). `net: None` models a
    ...
    /// so nothing downstream (BOM export, KiCad `np_thru_hole` writing,
    /// a future soldermask/keepout check) can mistake it for copper
    /// that merely happens to be unconnected right now -- see
    /// the development log's corresponding plan entry for the
    /// alternative (a plated stand-in) this was chosen over. `drill` is
...
///
/// `Sync`: `alladin-router`'s visibility-graph build (`astar.rs`'s
/// `build_adjacency_knn`/`build_adjacency_full`) shares a `&dyn
/// RuleResolver` across worker threads while computing candidate edges
/// in parallel -- a no-op requirement for both real implementors below
/// (`FixedClearance`, `JlcpcbClearance`), which are already plain,
...
/// KiCad's own `PNS::RULE_RESOLVER` makes (see the development log's note on
/// `MOCK_RULE_RESOLVER`), replacing the placeholder [`FixedClearance`]
...
/// -- callers pick a resolver explicitly (see the development log's
/// "Teil 22" entry for why `2layer_2oz` specifically, and why this is a
/// separate struct rather than a constructor parameter).
...
impl RuleResolver for JlcpcbClearance {
    fn clearance(&self, a: &Item, b: &Item) -> Unit {
        match (a, b) {
            ...
            // minimum (see the plan's Phase C design note -- there's no
            // separate "zone" row in JLCPCB's own capability table).
...
/// `Node::query_colliding`), and because most of them (silkscreen,
/// soldermask, `2layer_2oz`+ variants of *these* specific rows) have no
/// Rust-side consumer wired up yet -- see the development log's
/// "Elfter MVP-Slice" entry for exactly which of these are actually
/// enforced today versus documented-but-not-yet-checked.
...
pub struct Node {
    ...
    /// must also stay `Sync` so `&Node` can be shared across
    /// `alladin-router`'s parallel visibility-graph build (see
    /// `astar.rs`'s `build_adjacency_knn`/`build_adjacency_full`) --
    /// a `RefCell` here would make that impossible to even compile. The
    /// lock is only ever held briefly (a `HashMap` lookup or insert),
    /// never across a collision check.
    ///
...
impl Node {
    ...
    /// (removed items are silently skipped). Used by `alladin-router`'s
    /// A* visibility-graph search as the unfiltered fallback when
    /// [`Node::query_region`] doesn't cover every obstacle a route might
    /// need -- unlike [`Node::query_colliding`], this deliberately does
    /// *not* pre-filter by net/layer, since a graph edge that happens to
    /// pass near an irrelevant item is harmless (the real edge-validity
    /// check is still the exact [`Node::path_is_clear`] call, which does
    /// do that filtering).
    ...
    /// at (SHOVE's own "is this candidate blocker's net otherwise
    /// untouched by any neighbouring track?" check is the first real
    /// caller; see `alladin_router::shove`).
    ...
    /// spatial pre-filter `alladin-router`'s A* search uses to build its
    /// visibility graph from only the obstacles plausibly relevant to a
    /// given `from`→`to` corridor, instead of every item on the whole
    /// board. Same non-filtering-by-net/layer caveat as [`Node::iter`].
    /// Removed items are never returned -- they're gone from the R-tree
    /// itself, not just filtered here.
    ...
    /// [`Self::path_is_clear`] is called for every candidate edge in
    /// `alladin-router`'s A* visibility-graph construction -- often
    /// millions of times for a single net on a real, densely-populated
    /// board -- and the overwhelming majority of those calls are either
    /// answered by the very first nearby item (a genuine collision) or
    /// have to confirm *no* collision by checking every nearby item
    /// anyway, so the only real waste `query_colliding().is_empty()` was
    /// paying for here was the `Vec` allocation/push overhead on every
    /// single call, not the search itself -- removing it is a real,
    /// measured part of what fixed a router hang on a real board (see
    ...
    /// what happens hundreds or thousands of times over while
    /// `alladin-router`'s A* builds one net's visibility graph -- only
    /// ever pay the index's own `O(vertex count)` build cost once, not
    /// once per query (see [`PolygonEdgeIndex`]'s doc comment for why
    /// that distinction is what actually fixed a real router hang, not
    /// just a constant-factor speedup). Safe to cache for `Node`'s whole
    /// lifetime: zones are never mutated -- [`Self::replace`]/
    /// [`Self::remove`] are only ever used on `Track`/`Via` items by
    /// SHOVE -- so a cached index can never go stale.
    ...
    /// of. `alladin-pcb`'s `check_net_continuity` MCP tool is the first
    /// caller, reporting exactly this split back to the user/an AI
    /// client.
    ///
    ...
    /// This is the exact question Alladin's A* waypoint layer needs to
    /// ask before committing to a candidate edge in its search graph.
